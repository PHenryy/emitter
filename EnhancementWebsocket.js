...
class EnhancementWebSocket extends Emitter {
  ...
  onmessage(messageEvent) {
    ...
    if (this.options.pingEnabled) {
      if (isFunction(this.options.pongMessage)) {
        const pongReceived = this.options.pongMessage.call(null, messageEvent.data)
        if (pongReceived) {
    ...
    const data = this.options.stringifyMessage ? JSON.parse(messageEvent.data) : messageEvent.data
  ...
  send(data) {
    ...
    this.socket.send(this.options.stringifyMessage ? JSON.stringify(data) : data)
  }
